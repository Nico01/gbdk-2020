...
void list(void)
{
    ...
    else if (xflag == 1) { // OCTAL output Option.
        // Equate only
        if (lmode == ELIST) {
    ...
    else if (xflag == 2) { // DECIMAL output Option.
        // Equate only
        if (lmode == ELIST) {
...
void lstsym(FILE *fp)
{
    ...
    // Allocate space for an array of pointers to symbols and load array.
    if ((p = malloc(sizeof((struct sym *)sp) * nmsym)) == NULL) {

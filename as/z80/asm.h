...
extern jmp_buf jump_env;           // compiler dependent structure used by setjmp() and longjmp()
extern int inpfil;                 // count of assembler input files specified
extern int incfil;                 // current file handle index for include files
extern int cfile;                  // current file handle index of input assembly files
extern int flevel;                 // IF-ELSE-ENDIF flag will be non zero for false conditional case
extern int tlevel;                 // current conditional level
extern int ifcnd[MAXIF + 1];       // array of IF statement condition values (0 = FALSE) indexed by tlevel
extern int iflvl[MAXIF + 1];       // array of IF-ELSE-ENDIF flevel values indexed by tlevel
extern char afn[FILSPC];           // afile() temporary filespec
extern char srcfn[MAXFIL][FILSPC]; // array of source file names
extern int srcline[MAXFIL];        // current source file line
extern char incfn[MAXINC][FILSPC]; // array of include file names
extern int incline[MAXINC];        // current include file line
...
extern Addr_T laddr;               // address of current assembler line or value of .if argument
extern Addr_T fuzz;                /* tracks pass to pass changes in the address of symbols
                                    * caused by variable length instruction formats */
...
extern struct sym *symhash[NHASH]; // array of pointers to NHASH linked symbol lists
extern struct mne *mnehash[NHASH]; // array of pointers to NHASH linked mnemonic/directive lists
extern char *ep;                   // pointer into error list array eb[NERR]
extern char eb[NERR];              // array of generated error codes
extern char *ip;                   // pointer into the assembler-source text line in ib[]
extern char ib[NINPUT];            // assembler-source text line
extern char *cp;                   // pointer to assembler output array cb[]
extern char cb[NCODE];             // array of assembler output values
extern int *cpt;                   // pointer to assembler relocation type output array cbt[]
extern int cbt[NCODE];             // array of assembler relocation types describing the data in cb[]
...
extern char ctype[128];            // array of character types, one per ASCII character


#if CASE_SENSITIVE
#else
extern char ccase[128]; // an array of characters which perform the case translation function
#endif
...
/*
 * The exp structure is used to return the evaluation of an expression.
 * The structure supports three valid cases:
 *  (1) The expression evaluates to a constant,
 *  mode = S_USER, flag = 0, addr contains the constant, and base = NULL.
 *  (2) The expression evaluates to a defined symbol plus or minus a constant, 
 *  mode = S_USER, flag = 0, addr contains the constant, and base = pointer to area symbol.
 *  (3) The expression evaluates to a external global symbol plus or minus a constant,
 *  mode = S_NEW, flag = 1, addr contains the constant, and base = pointer to symbol.
 */
struct expr {

...
jmp_buf jump_env;               //compiler dependent structure used by setjmp() and longjmp()
int inpfil;                     //count of assembler input files specified
int incfil;                     //current file handle index for include files
int cfile;                      //current file handle index of input assembly files
int flevel;                     //IF-ELSE-ENDIF flag will be non zero for false conditional case
int tlevel;                     //current conditional level
int ifcnd[MAXIF+1];             //array of IF statement condition values (0 = FALSE) indexed by tlevel
int iflvl[MAXIF+1];             //array of IF-ELSE-ENDIF flevel values indexed by tlevel
...
Addr_T laddr;                   //address of current assembler line or value of .if argument
Addr_T fuzz;                    /* tracks pass to pass changes in the address of symbols
                                 * caused by variable length instruction formats */
int lmode;                      //listing mode
char *ep;                       //pointer into error list array eb[NERR]
char eb[NERR];                  //array of generated error codes
char *ip;                       //pointer into the assembler-source text line in ib[]
char ib[NINPUT];                //assembler-source text line
char *cp;                       //pointer to assembler output array cb[]
char cb[NCODE];                 //array of assembler output values
int *cpt;                       //pointer to assembler relocation type output array cbt[]
int cbt[NCODE];                 //array of assembler relocation types describing the data in cb[]
 ...
 * };
 */
struct sym sym[] = {{ NULL, NULL, ".", S_USER, S_END, NULL, 0, }};

struct sym *symp;           // pointer to a symbol structure

struct sym *symhash[NHASH]; // array of pointers to NHASH linked symbol lists
 ...
 * };
 */
struct area area[] = {{ NULL, "_CODE", 0, 0, 0, A_CON|A_REL }};
...
// array of character types, one per ASCII character
...
// an array of characters which perform the case translation function
